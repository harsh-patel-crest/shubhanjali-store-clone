...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border">
      <div className="text-center text-xs sm:text-sm py-2 px-4" style={{ backgroundColor: "#4f9d79", color: "#FFFFFF" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 flex items-center gap-4 h-20">
        ...
        <div className="ml-auto flex items-center gap-2 sm:gap-4">
          ...
          <Link to="/cart" className="relative p-2">
            ...
            <span className="absolute -top-0.5 -right-0.5 text-[10px] rounded-full h-4 w-4 flex items-center justify-center" style={{ backgroundColor: "#4d9f7a", color: "#FFFFFF" }}>{count}</span>
          </Link>
        </div>
      </div>
      {open && (
